...
# keeps the script usable when launched as ``python utils/eval_question_only.py``.
REPO_ROOT = Path(__file__).resolve().parent.parent
PROMPT_PATH = REPO_ROOT / "prompts" / "question_only.prompt"
DATA_PATH = REPO_ROOT / "data" / "gsm8k" / "train.jsonl"
RESULT_PATH = REPO_ROOT / "results" / "eval result" / "question_only_gsm8k_base_eval.jsonl"
...
with RESULT_PATH.open("w", encoding="utf-8") as f:
    for start in tqdm(
        ...
        for j, completion in enumerate(batch_completions):
            ...
            result = grader.question_only_reward_fn(
                completion.text,
